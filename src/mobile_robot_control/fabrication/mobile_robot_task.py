...
__all__ = [
    "MobileRobotGetMarkerPoseTask",
    "MobileRobotHomeConfigurationTask",
    "MobileRobotMarkerSnapshotTask",
    "MobileRobotPickPlaceTask"
]

class MobileRobotGetMarkerPoseTask(Task):
    def __init__(self, robot, marker_id="marker_0", reference_frame_id="base", key=None):
        super(MobileRobotGetMarkerPoseTask, self).__init__(key)
        self.robot = robot
    ...
    def run(self, stop_thread):
        ...
        if self.robot.mobile_client.tf_frame is None:
            self.log('Could not get the frame.')
        self.robot.mobile_client.tf_unsubscribe(self.marker_id, self.reference_frame_id)
        self.is_completed = True
        return True

class MobileRobotHomeConfigurationTask(URTask):
    def __init__(self, robot, robot_address, velocity=0.016, key=None):
        super(MobileRobotHomeConfigurationTask, self).__init__(robot, robot_address, key)
    ...
    def run(self, stop_thread):
        ...
        super(MobileRobotHomeConfigurationTask, self).run(stop_thread)
               
class MobileRobotMarkerSnapshotTask(URTask):
    def __init__(self, robot, robot_address, routine=0, marker_id='marker_0', target_marker_id='marker_1', velocity=0.016, radius=0.01, key=None):
        super(MobileRobotMarkerSnapshotTask, self).__init__(robot, robot_address, key)
    ...
    def run(self, stop_thread):
        ...
        super(MobileRobotMarkerSnapshotTask, self).run(stop_thread)
        
        
class MobileRobotPickPlaceTask(URTask):
    def __init__(self, robot, robot_address, brick_no=1, marker_id='marker_0', target_marker_id='marker_1', velocity=0.016, radius=0.01, key=None):
        super(MobileRobotPickPlaceTask, self).__init__(robot, robot_address, key)
        self.marker_id = marker_id
        self.target_marker_id = target_marker_id
        self.velocity = velocity
        self.radius = radius
    ...
    def create_urscript_for_pick_place(self, marker_frame_1, marker_frame_2):
        ...
        frame_2_safe = frame_2.transformed(Translation.from_vector(-frame_2.yaxis*0.15) * Translation.from_vector(Vector.Zaxis()*(self.brick_no*0.05+0.1)))
        
        self.log("From {} to {}.".format(frame_1, frame_2))
        tool_angle_axis = list(self.robot.attached_tool.frame.point) + list(self.robot.attached_tool.frame.axis_angle_vector)
    ...
    def run(self, stop_thread):
        ...
        super(MobileRobotPickPlaceTask, self).run(stop_thread)

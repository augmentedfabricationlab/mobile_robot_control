...
def updateSearchPaths(settings_file, python_source_path):
    with codecs.open(
        settings_file, "r", encoding="ascii", errors="ignore"
    ) as file_handle:
...
def clean(ctx, docs=True, bytecode=True, builds=True):
    ...
    if bytecode:
        folders.append("src/mobile_additive_manufacturing/__pycache__")

    if builds:
        folders.append("build/")
        folders.append("src/mobile_additive_manufacturing.egg-info/")
